...
function GetSendCard($formname,$nickname)
{
	...
	if(!isset($_GET["lid"]))
	{
		if($formname=="resource_specialty_sendcard")
		$sql="select ssendcard_id,user_account,user_nickname,ssendcard_title,ssendcard_content,ssendcard_datetime 
		 from resource_specialty_sendcard,users where ssendcard_users=user_account order by ssendcard_id DESC";
		$res=mysqli_query($mysqli,$sql);
	...
	else
	{
		if($formname=="resource_specialty_sendcard")
		$sql="select ssendcard_id,user_account,user_nickname,ssendcard_title,ssendcard_content,ssendcard_datetime 
		 from resource_specialty_sendcard,users where ssendcard_users=user_account 
		 and ssendcard_id in (select ssendcard_id from resource_specialty_cardlabel where slabel_id=".$_GET["lid"].")order by ssendcard_id DESC";
		$res=mysqli_query($mysqli,$sql);
		
	}
	if($res&&$formname=="resource_specialty_sendcard")
	{
		$resultarray=array();
		$index=0;
		...
		while($resultarray[$index])
		{
			...
			if($nickname!="未登录")
			echo"<h3><a href=\"resource_appeal_browse.html?sid=".$_GET["sid"]."&cid=".$resultarray[$index]["ssendcard_id"]."\"><strong><i>".$resultarray[$index]["ssendcard_title"]."</i></strong></a></h3>";
			else
			echo"<h3><a href=\"resource_appeal_browse.html?cid=".$resultarray[$index]["ssendcard_id"]."\"><strong><i>".$resultarray[$index]["ssendcard_title"]."</i></strong></a></h3>";
			echo"<p>".$content."</p>";
			$sql2="select count(ssendcard_users) from resource_specialty_like where ssendcard_id=".$resultarray[$index]["ssendcard_id"];
			...
			if($nickname!="未登录")
			{
				$sql2="select * from resource_specialty_like where ssendcard_id=".$resultarray[$index]["ssendcard_id"]." and ssendcard_users='".$_SESSION["account"]."'";
				$res2=mysqli_query($mysqli,$sql2);
...
function like($cardid)
{
	...
	$mysqli->query("SET NAMES utf8");
	$sql="insert into resource_specialty_like values(".$cardid.",'".$_SESSION["account"]."')";
	$res=mysqli_query($mysqli,$sql);
	if($res)
		return true;
	else
...
function ShowSendCard($formname,$cardid)
{
	...
	if($formname=="resource_specialty_sendcard")
	$sql="select ssendcard_id,user_account,user_nickname,ssendcard_title,ssendcard_content,ssendcard_datetime 
		 from resource_specialty_sendcard,users where ssendcard_users=user_account and  ssendcard_id=".$cardid;
	$res=mysqli_query($mysqli,$sql);
	if($res&&$formname=="resource_specialty_sendcard")
	{
...
function ShowFollowCard($formname,$cardid)
{
	...
	$mysqli->query("SET NAMES utf8");
	if($formname=="resource_specialty_followcard")
	$sql="select user_nickname,sfollowcard_content,sfollowcard_datetime 
		 from resource_specialty_followcard,users where sfollowcard_users=user_account and sfollowcard_sendcard_id=".$cardid." order by sfollowcard_id";
	$res=mysqli_query($mysqli,$sql);
	if($res)
	{
...
function SendCard($formname)
{
	...
	if($formname=="resource_specialty_sendcard")
	{
		$sql="insert into resource_specialty_sendcard values(null,'".$_SESSION["account"]."','".$_POST["title"]."','".$_POST["content"]."','".$datetime."')";
		$res=mysqli_query($mysqli,$sql);
		if($res)
		{
			$sql="select max(ssendcard_id) from resource_specialty_sendcard";
			$res=mysqli_query($mysqli,$sql);
...
function FollowCard($formname,$cardid)
{
	...
	if($formname=="resource_specialty_followcard")
	{
		$sql="insert into resource_specialty_followcard values(null,".$cardid.",'".$_SESSION["account"]."','".$_POST["followcard"]."','".$datetime."')";
		$res=mysqli_query($mysqli,$sql);
		if($res)
			return true;
	}
