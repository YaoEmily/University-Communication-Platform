...
function PrintLabel($formname,$nickname)
{
	...
	if($res&&$formname=="school_label")
	{
		while($labelarray=mysqli_fetch_array($res,MYSQLI_ASSOC))
		{
				...
				if($nickname!="未登录")
				echo"<li><a href=\"recreation_society.html?sid=".$_GET["sid"]."&lid=".$labelid."\"><i class=\"blog_icon4\"> </i><span>".$labelname."</span></a></li>";
				else
				echo"<li><a href=\"recreation_society.html?lid=".$labelid."\"><i class=\"blog_icon4\"> </i><span>".$labelname."</span></a></li>";
		}
	}
	//mysqli_close($res);
}
...
function GetSendCard($formname,$nickname)
{
	...
	if(!isset($_GET["lid"]))
	{
		if($formname=="recreation_society_sendcard")
		$sql="select ssendcard_id,user_account,user_nickname,ssendcard_title,ssendcard_content,ssendcard_datetime from recreation_society_sendcard,users where ssendcard_users=user_account order by ssendcard_id DESC";
		$res=mysqli_query($mysqli,$sql);
	...
	else
	{
		...
		$sql="select ssendcard_id,user_account,user_nickname,ssendcard_title,ssendcard_content,ssendcard_datetime 
		 from recreation_society_sendcard,users where ssendcard_users=user_account 
		 and ssendcard_id in (select ssendcard_id from recreation_society_cardlabel where slabel_id=".$_GET["lid"].")order by ssendcard_id DESC";
		$res=mysqli_query($mysqli,$sql);
		
	}
	if($res&&$formname=="recreation_society_sendcard")
	{
		$resultarray=array();
		$index=0;
		...
		while($resultarray[$index])
		{
			...
			if($nickname!="未登录")
			echo"<h3><a href=\"recreation_society_browse.html?sid=".$_GET["sid"]."&cid=".$resultarray[$index]["ssendcard_id"]."\"><strong><i>".$resultarray[$index]["ssendcard_title"]."</i></strong></a></h3>";
			else
			echo"<h3><a href=\"recreation_society_browse.html?cid=".$resultarray[$index]["ssendcard_id"]."\"><strong><i>".$resultarray[$index]["ssendcard_title"]."</i></strong></a></h3>";
			echo"<p>".$content."</p>";
			$sql2="select count(ssendcard_users) from recreation_society_like where ssendcard_id=".$resultarray[$index]["ssendcard_id"];
			...
			if($nickname!="未登录")
			{
				$sql2="select * from recreation_society_like where ssendcard_id=".$resultarray[$index]["ssendcard_id"]." and ssendcard_users='".$_SESSION["account"]."'";
				$res2=mysqli_query($mysqli,$sql2);
			...
			(select slabel_id from recreation_society_cardlabel where ssendcard_id=".$resultarray[$index]["ssendcard_id"].")";
			$res2=mysqli_query($mysqli,$sql2);
			if($res2)
			{
...
function like($cardid)
{
	...
	$mysqli->query("SET NAMES utf8");
	$sql="insert into recreation_society_like values(".$cardid.",'".$_SESSION["account"]."')";
	$res=mysqli_query($mysqli,$sql);
	if($res)
		return true;
	else
...
function ShowSendCard($formname,$cardid)
{
	...
	if($formname=="recreation_society_sendcard")
	{
		$sql="select ssendcard_id,user_account,user_nickname,ssendcard_title,ssendcard_content,ssendcard_datetime from recreation_society_sendcard,users where ssendcard_users=user_account and  ssendcard_id=".$cardid;
	}
	$res=mysqli_query($mysqli,$sql);
	if($res&&$formname=="recreation_society_sendcard")
	{
...
function ShowFollowCard($formname,$cardid)
{
	...
	$mysqli->query("SET NAMES utf8");
	if($formname=="recreation_society_followcard")
	$sql="select user_nickname,sfollowcard_content,sfollowcard_datetime 
		 from recreation_society_followcard,users where sfollowcard_users=user_account and sfollowcard_sendcard_id=".$cardid." order by sfollowcard_id";
	$res=mysqli_query($mysqli,$sql);
	if($res)
	{
...
function SendCard($formname)
{
	...
	if($formname=="recreation_society_sendcard")
	{
		$sql="insert into recreation_society_sendcard values(null,'".$_SESSION["account"]."','".$_POST["title"]."','".$_POST["content"]."','".$datetime."')";
		$res=mysqli_query($mysqli,$sql);
		if($res)
		{
			$sql="select max(ssendcard_id) from recreation_society_sendcard";
			$res=mysqli_query($mysqli,$sql);
			if($res&&isset($_POST["label"]))
			{
				...
				{
					$sql="insert into recreation_society_cardlabel values(".$card["max(ssendcard_id)"].",".$label.")";
					$res=mysqli_query($mysqli,$sql);
				}
				if($res)
					return true;
...
function FollowCard($formname,$cardid)
{
	...
	if($formname=="recreation_society_followcard")
	{
		$sql="insert into recreation_society_followcard values(null,".$cardid.",'".$_SESSION["account"]."','".$_POST["followcard"]."','".$datetime."')";
		$res=mysqli_query($mysqli,$sql);
		if($res)
			return true;
	}
