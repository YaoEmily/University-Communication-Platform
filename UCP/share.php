...
function ShowFollowCard($formname,$cardid)
{
	...
	$mysqli->query("SET NAMES utf8");
	if($formname=="resource_share_followcard")
	$sql="select user_nickname,sfollowcard_content,sfollowcard_datetime 
		 from resource_share_followcard,users where sfollowcard_users=user_account and sfollowcard_sendcard_id=".$cardid." order by sfollowcard_id";
	$res=mysqli_query($mysqli,$sql);
	if($res)
	{
...
function FollowCard($formname,$cardid)
{
	...
	if($formname=="resource_share_followcard")
	{
		$sql="insert into resource_share_followcard values(null,".$cardid.",'".$_SESSION["account"]."','".$_POST["followcard"]."','".$datetime."')";
		$res=mysqli_query($mysqli,$sql);
		if($res)
			return true;
	}
