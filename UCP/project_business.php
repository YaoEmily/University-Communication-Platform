...
function GetProjectName($cardid)
{
	header ( "Content-Type: text/html; charset=utf8" );
	$mysqli=mysqli_connect("localhost","root","hhh19950129","uni_com_platform");
	if(mysqli_connect_error())
	...
	{
		$administrator=mysqli_fetch_array($res,MYSQLI_ASSOC);
		return $administrator["ssendcard_title"];
	}
	return false;
}
function GetProjectUsers($cardid)
{
	header ( "Content-Type: text/html; charset=utf8" );
	$mysqli=mysqli_connect("localhost","root","hhh19950129","uni_com_platform");
	if(mysqli_connect_error())
	...
	$mysqli->query("SET NAMES utf8");
	$sql="select ssendcard_id,ssendcard_users,user_account,user_nickname,ssendcard_title,ssendcard_datetime,ssendcard_school,ssendcard_place,ssendcard_people,ssendcard_description,ssendcard_requirement,ssendcard_mobile,ssendcard_phone,ssendcard_email,ssendcard_qq from project_business_sendcard,users where ssendcard_users=user_account and  ssendcard_id=".$cardid;
	$res=mysqli_query($mysqli,$sql);
	if($res)
	{
		$administrator=mysqli_fetch_array($res,MYSQLI_ASSOC);
		return $administrator["ssendcard_users"];
	}
	return false;
}
function GetPlace($cardid)
{
	header ( "Content-Type: text/html; charset=utf8" );
	$mysqli=mysqli_connect("localhost","root","hhh19950129","uni_com_platform");
	if(mysqli_connect_error())
	...
	{
		$administrator=mysqli_fetch_array($res,MYSQLI_ASSOC);
		return $administrator["ssendcard_place"];
	}
	return false;
}
function GetPeople($cardid)
{
	header ( "Content-Type: text/html; charset=utf8" );
	$mysqli=mysqli_connect("localhost","root","hhh19950129","uni_com_platform");
	if(mysqli_connect_error())
	...
	{
		$administrator=mysqli_fetch_array($res,MYSQLI_ASSOC);
		return $administrator["ssendcard_people"];
	}
	return false;
}
function GetDescription($cardid)
{
	...
	{
		$administrator=mysqli_fetch_array($res,MYSQLI_ASSOC);
		return $administrator["ssendcard_description"];
	}
	return false;
}
function GetRequirement($cardid)
{
	header ( "Content-Type: text/html; charset=utf8" );
	$mysqli=mysqli_connect("localhost","root","hhh19950129","uni_com_platform");
	if(mysqli_connect_error())
	...
	{
		$administrator=mysqli_fetch_array($res,MYSQLI_ASSOC);
		return $administrator["ssendcard_requirement"];
	}
	return false;
}
...
function GetPhone($cardid)
{
	...
	{
		$administrator=mysqli_fetch_array($res,MYSQLI_ASSOC);
		return $administrator["ssendcard_phone"];
	}
	return false;
}
...
function PrintLabel($formname,$nickname)
{
	...
	if($res&&$formname=="school_label")
	{
		while($labelarray=mysqli_fetch_array($res,MYSQLI_ASSOC))
		{
				...
				if($nickname!="未登录")
				echo"<li><a href=\"project_business.html?sid=".$_GET["sid"]."&lid=".$labelid."\"><i class=\"blog_icon4\"> </i><span>".$labelname."</span></a></li>";
				else
				echo"<li><a href=\"project_business.html?lid=".$labelid."\"><i class=\"blog_icon4\"> </i><span>".$labelname."</span></a></li>";
		}
	}
	//mysqli_close($res);
}
...
function GetSendCard($formname,$nickname)
{
	...
	if(!isset($_GET["lid"]))
	{
		if($formname=="project_business_sendcard")
		$sql="select ssendcard_id,user_account,user_nickname,ssendcard_title,ssendcard_description,ssendcard_datetime from project_business_sendcard,users where ssendcard_users=user_account order by ssendcard_id DESC";
		$res=mysqli_query($mysqli,$sql);
	...
	{
		if($formname=="project_business_sendcard")
		$sql="select ssendcard_id,user_account,user_nickname,ssendcard_title,ssendcard_description,ssendcard_datetime 
		 from project_business_sendcard,users where ssendcard_users=user_account 
		 and ssendcard_id in (select ssendcard_id from project_business_cardlabel where slabel_id=".$_GET["lid"].")order by ssendcard_id DESC";
		$res=mysqli_query($mysqli,$sql);
		
	}
	if($res&&$formname=="project_business_sendcard")
	{
		$resultarray=array();
		$index=0;
		...
		{
			echo"<hr/>";
			$content=mb_substr($resultarray[$index]["ssendcard_description"] , 0 , 125); 
			echo"<li>";
			echo"<img src=\"".$resultarray[$index]["user_account"]."\headImg.jpg\" alt=\"\"/ style=\"height:135px; width:113px\">";
			if($nickname!="未登录")
			echo"<h3><a href=\"project_business_browse.html?sid=".$_GET["sid"]."&cid=".$resultarray[$index]["ssendcard_id"]."\"><strong><i>".$resultarray[$index]["ssendcard_title"]."</i></strong></a></h3>";
			else
			echo"<h3><a href=\"project_business_browse.html?cid=".$resultarray[$index]["ssendcard_id"]."\"><strong><i>".$resultarray[$index]["ssendcard_title"]."</i></strong></a></h3>";
			echo"<p>".$content."</p>";
			$sql2="select count(ssendcard_users) from project_business_like where ssendcard_id=".$resultarray[$index]["ssendcard_id"];
			...
			if($nickname!="未登录")
			{
				$sql2="select * from project_business_like where ssendcard_id=".$resultarray[$index]["ssendcard_id"]." and ssendcard_users='".$_SESSION["account"]."'";
				$res2=mysqli_query($mysqli,$sql2);
			...
			(select slabel_id from project_business_cardlabel where ssendcard_id=".$resultarray[$index]["ssendcard_id"].")";
			$res2=mysqli_query($mysqli,$sql2);
			if($res2)
			{
...
function like($cardid)
{
	...
	$mysqli->query("SET NAMES utf8");
	$sql="insert into project_business_like values(".$cardid.",'".$_SESSION["account"]."')";
	$res=mysqli_query($mysqli,$sql);
	if($res)
		return true;
	else
...
function ShowFollowCard($formname,$cardid)
{
	...
	$mysqli->query("SET NAMES utf8");
	if($formname=="project_business_followcard")
	$sql="select user_nickname,sfollowcard_content,sfollowcard_datetime 
		 from project_business_followcard,users where sfollowcard_users=user_account and sfollowcard_sendcard_id=".$cardid." order by sfollowcard_id";
	$res=mysqli_query($mysqli,$sql);
	if($res)
	{
...
function SendCard($formname)
{
	...
	if($formname=="project_business_sendcard")
	{
		$sql="insert into project_business_sendcard values(null,'".$_SESSION["account"]."','".$_POST["title"]."','".$datetime."','".$_POST["project_school"]."','".$_POST["project_place"]."','".$_POST["project_people"]."','".$_POST["project_description"]."','".$_POST["project_requirement"]."','".$_POST["project_mobile"]."','".$_POST["project_phone"]."','".$_POST["project_email"]."','".$_POST["project_qq"]."')";
		$res=mysqli_query($mysqli,$sql);
		if($res)
		{
			$sql="select max(ssendcard_id) from project_business_sendcard";
			$res=mysqli_query($mysqli,$sql);
			if($res&&isset($_POST["label"]))
			{
				...
				{
					$sql="insert into project_business_cardlabel values(".$card["max(ssendcard_id)"].",".$label.")";
					$res=mysqli_query($mysqli,$sql);
				}
				if($res)
					return true;
...
function FollowCard($formname,$cardid)
{
	...
	if($formname=="project_business_followcard")
	{
		$sql="insert into project_business_followcard values(null,".$cardid.",'".$_SESSION["account"]."','".$_POST["followcard"]."','".$datetime."')";
		$res=mysqli_query($mysqli,$sql);
		if($res)
			return true;
	}
